...
Status: **Decided (Phase 3)** — zero data dependency for the deployed site;
optional feature-gated local SQLite index for editor tooling.

## Decision

- The **deployed site has no database**. It is a fully static build: pages,
  Devlog, feeds, sitemap and `search-index.json` are generated by the canonical
  Node pipeline from Markdown + YAML frontmatter. This is a hard requirement,
  not a preference.
...
- Non-goals (explicitly rejected, matching the master prompt):
  - a client/server database for the site;
  - making SQLite the canonical article format (Markdown remains canonical);
